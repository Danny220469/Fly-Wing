...
try:
    final_df = pd.read_csv("lda_results.csv")
    variance_df = pd.read_csv("lda_variance.csv")
except FileNotFoundError as e:
    print(f"Error: Could not find '{e.filename}'.")
    print("Please run the 'perform_lda.R' script first.")
    exit()

var_ld1 = variance_df['var_LD1'].iloc[0]
var_ld2 = variance_df['var_LD2'].iloc[0]
var_ld3 = variance_df['var_LD3'].iloc[0]


# --- 2. Create the 3D LDA Plot ---
...
fig = px.scatter_3d(
    final_df, x='LD1', y='LD2', z='LD3',
    color='species', symbol='sex', symbol_map=symbol_map, color_discrete_map=color_map,
    title="3D LDA of Fly Wing Harmonics (Hover to Highlight, Click for Details)",
    labels={
        'LD1': f'LD1 ({var_ld1:.1%})',
        'LD2': f'LD2 ({var_ld2:.1%})',
        'LD3': f'LD3 ({var_ld3:.1%})'
