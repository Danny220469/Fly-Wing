...
calculate_sscp_percent <- function(dependent_vars, factors) {
    ...
    # --- MODEL 2 FORMULA ---
    # This formula focuses on the species:sex interaction.
    formula_str <- paste("cbind(", paste(colnames(dependent_vars), collapse = ","), ") ~ family + species * sex")
...
print("=== Comparison of SSCP Percentage Contributions (with Species:Sex) ===")
print(df_compare)
...
plot <- ggplot(df_long, aes(x = analysis, y = contribution, fill = term)) +
    ...
    labs(
        title = "SSCP Contribution (Model with species:sex Interaction)",
        y = "Contribution (%)",
        x = "Analysis Method",
        fill = "Model Term"
    ) +
...
ggsave("sscp_comparison_species_sex_interaction.png", plot = plot, width = 14, height = 8, units = "in", dpi = 300)

print("✅ Generated plot 2: sscp_comparison_species_sex_interaction.png")